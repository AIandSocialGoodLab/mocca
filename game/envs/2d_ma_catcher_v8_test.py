...
"""
ty: this should only be used for testing cycle shaped defender
	...
	def _calcReward(self):
		...
		for i in range(self.num_target):
			...
			if (tarAttDist <= 15.0) and (attDefDist <= 20.0):
				...
				return (-tarR, tarR), True, {"done": "target attacked"} #ty: focus on reaching target
			elif (tarAttDist <= 15.0) and (self.penetrationCtrs[i] < self.penetrationTime):
				# attacker ongoing attack
				self.penetrationCtrs[i] += 1
				return (-0.5, 1.0), False, {"done": None}
			else:
				# other cases
				self.penetrationCtrs[i] = 0
		
		return (-0.5, -0.5), False, {"done": None}
