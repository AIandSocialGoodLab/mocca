...
class CatcherEnv(gym.Env):
	...
	def __init__(self):
		...
		self.action_space = gym.spaces.Box(low=np.array([0,0,0,0],dtype=np.float32), high=np.array([360, 20, 360, 20],dtype=np.float32))
		# attacker x, y
	...
	def _tanhOut2degree(self, out):

		return (out + 1) * 360 / 2 + 0
	...
	def step(self, action):
		...
		# TODO: vectorize this
		action[0] = self._tanhOut2degree(action[0])
		action[1] = np.clip(self._tanhOut2distance(action[1]), 0, 20)
		action[2] = self._tanhOut2degree(action[2])
		action[3] = np.clip(self._tanhOut2distance(action[3]), 0, 20)

		
		# TODO: vectorize this
		defDeltax = np.sin(self._degree2radian(action[0])) * action[1]
		defDeltay = np.cos(self._degree2radian(action[0])) * action[1]

		uavDeltax = np.sin(self._degree2radian(action[2])) * action[3]
		uavDeltay = np.cos(self._degree2radian(action[2])) * action[3]
