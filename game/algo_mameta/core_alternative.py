...
"""
same as core only differences:  
1. return std & log std as well
2. clip mu and pi
"""
...
def mlp_gaussian_policy(x, a, hidden_sizes, activation, output_activation):
    ...
    return mu, pi, logp_pi, log_std # ty: only difference
...
def mlp_actor_critic(x, a, hidden_sizes=(256,256), activation=tf.nn.relu, 
                     ...
                     centralizeQ=False, oa=None):
    # policy
    with tf.variable_scope('pi'):
        mu, pi, logp_pi, log_std = policy(x, a, hidden_sizes, activation, output_activation)
    ...
    with tf.variable_scope('q1'):
        if not centralizeQ:
            ...
            q1 = vf_mlp(tf.concat([x,a,oa], axis=-1))
    with tf.variable_scope('q2'):
        if not centralizeQ:
            q2 = vf_mlp(tf.concat([x,a], axis=-1))
        else:
            q2 = vf_mlp(tf.concat([x,a,oa], axis=-1))
    mu = tf.clip_by_value(mu, -20, 20)
    pi = tf.clip_by_value(pi, -20, 20)
    return mu, pi, logp_pi, q1, q2, log_std
